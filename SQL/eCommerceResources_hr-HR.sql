...
IF (@ResourceTypeID > 0 AND @LanguageID > 0) BEGIN
	...
	SELECT N'AccountLocked_message', N'Račun je zaključan. Istekla vam je licenca. Za nastavak korištenja aplikacije, molimo obnovite licencu', N'Your account is locked because your license has been expired. To continue using this App, please renew your license.' UNION ALL
		...
		SELECT N'Payment_Credit_Card_Warning', N'Plaćanje možete izvršiti koristeći kreditnu karticu ili PayPal-om. Unesite broj kartice, zajedno s važećim CVC-om i datumom isteka, kako biste sigurno dovršili transakciju.', N'You can make your payment using either a credit card or PayPal. Please enter your card number, along with a valid CVC and expiration date, to complete the transaction securely.' UNION ALL
		SELECT N'Payment_Credit_Card_Test_Warning', N'Plati bankovnom karticom. TEST MOD JE OMOGUĆEN. U test modu možete koristiti broj kartice 0000 0000 0000 0000 sa bilo kojim CVC brojem kao i podacima o isteku kartice.', N'Pay with your credit card. TEST MODE ENABLED. In test mode, you can use the card number 0000 0000 0000 0000 with any CVC and a valid expiration date.' UNION ALL
		...
		SELECT N'PaymentSettings_PaymentsPageSettings', N'Linkovi za uvjete plaćanja', N'Payments Page Settings' UNION ALL
		SELECT N'PaymentSettings_PrivacyPage', N'Stranica pravila privatnosti', N'Privacy Policy Page' UNION ALL
		SELECT N'PaymentSettings_RefundPage', N'Stranica uvjeti povrata i reklamacije', N'Refund/Return Policy Page' UNION ALL
		SELECT N'PaymentSettings_TermsConditionPage', N'Stranica uvjeti poslovanja', N'Terms and Conditions Page' UNION ALL
